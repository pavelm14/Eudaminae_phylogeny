...
```bash
[pavel local]$ ssh -x pavelmatos@nympha.metacentrum.cz #To run NGS data analyses
[pavel local]$ ssh -x pavelmatos@zuphux.cerit-sc.cz #To run high-memory demanding jobs
[pavel local]$ ssh -x pavelmatos@skirit.metacentrum.cz #To run diversification and biogeography analyses
[pavel local]$ ssh -x pavelmatos@storage-brno3-cerit.metacentrum.cz #To back up data. This is the same as @zuphux.cerit-sc.cz but can't run any job here
```
...
```bash
[pavel local]$ cd /drives/d/PAVEL/Lepidoptera\ projects/Hesperiidae/Laboratory/RapidGenomics/data/raw/
```
...
```bash
[pavel raw]$ scp -r ./*.fastq.gz pavelmatos@zuphux.cerit-sc.cz:/home/pavelmatos/eudaminae/raw
```
